...
async def entrypoint(ctx: agents.JobContext):
    ...
    audio_path = Path(__file__).parent / "Orlando Scooter (1).wav"

    TARGET_RATE = 16000       # 16 kHz
    TARGET_WIDTH = 2          # 2 bytes = 16-bit PCM
    ...
    async def audio_generator():
        ...
        try:
            ...
            while True:
                ...
                # 1) convert sample width
                if src_width != TARGET_WIDTH:
                    raw = audioop.lin2lin(raw, src_width, TARGET_WIDTH)

                # 2) convert to mono
                if src_channels != TARGET_CHANNELS:
                    raw = audioop.tomono(raw, TARGET_WIDTH, 1, 1)

                # 3) resample to 16kHz
                if src_rate != TARGET_RATE:
                    raw, resample_state = audioop.ratecv(
                        raw, TARGET_WIDTH, TARGET_CHANNELS,
                        src_rate, TARGET_RATE, resample_state
                    )

                # Compute number of samples per channel
                samples_per_channel = len(raw) // (TARGET_WIDTH * TARGET_CHANNELS)
            ...
            wf.close()
    await agent.session.say("Playing audio file", audio=audio_generator())
